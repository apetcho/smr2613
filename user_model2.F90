...
!
      ...
      integer(ESMF_KIND_I4), dimension(:,:), pointer :: mskPtr
...
!
      ...
      if (file_exists) then
        ...
        call ESMF_ConfigFindLabel(cf, 'gcomp2_tile:', rc=rc)
        do i = 1, 2
        ...
        call ESMF_ConfigFindLabel(cf, 'gcomp2_grid:', rc=rc)
...
!
      ...
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,    &
          line=__LINE__, file=__FILE__))                                &
          call ESMF_Finalize(endflag=ESMF_END_ABORT)
      call ESMF_ArrayGet(centers_array_msk, localDe=0, farrayPtr=mskPtr, rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,    &
          line=__LINE__, file=__FILE__))                                &
          call ESMF_Finalize(endflag=ESMF_END_ABORT)
!
...
!
      call ESMF_FieldWrite(field1, 'gcomp2_data_imp.nc',   &
      ...
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,    &
          ...
          call ESMF_Finalize(endflag=ESMF_END_ABORT)
!
      call ESMF_FieldWrite(field2, 'gcomp2_data_exp.nc',   &
                           variableName='data', overwrite=.true.,   &
                           rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,    &
          line=__LINE__, file=__FILE__))                                &
...
!-----------------------------------------------------------------------
!
      call ESMF_GridWriteVTK(grid, filename="ocn_grd", rc=rc)
      if (ESMF_LogFoundError(rcToCheck=rc, msg=ESMF_LOGERR_PASSTHRU,    &
          line=__LINE__, file=__FILE__))                                &
          call ESMF_Finalize(endflag=ESMF_END_ABORT)
!
